...
public final class AutomationRunContext {
     ...
     * @param proxySet
     */
    public void cleanUpRunRequests(ProxySet proxySet) {
        ...
        synchronized (requests) {
            ...
            if(requestsIterator.hasNext()) {
                ...
                while(requestsIterator.hasNext())
                {
                    ...
                    if(!isRunOld(request)) {
                        log.info(String.format("Run [%s] is not at least [%d] seconds old.  Will not analyze.",targetUuid,AutomationRunContext.CLEANUP_LIFE_LENGTH_IN_SECONDS));
                        continue;
                    }
                    ...
                    for(RemoteProxy proxy : proxySet) {
                        ...
                        for(int i = 0;!uuidFound && i< slots.size(); i++) {
                            ...
                            if(testSession != null) {
                                // Check the session UUID instead of the node UUID as the node UUID is going to be the test run UUID that caused
                                // the node to be started, but will not necessarily be the run that is currently running on the node
                                Object sessionUuid = testSession.getRequestedCapabilities().get(AutomationConstants.UUID);
                    ...
                    if(!uuidFound) {
                        log.info(String.format("Tracked test run [%s] found with no running tests.  Adding to set for removal.",targetUuid));
    ...
    /**
     * Returns the number of additional threads this hub can support.  This considers all
    ...
    public int getTotalThreadsAvailable(ProxySet proxySet) {
        ...
        if(proxySet != null) {
            for(RemoteProxy proxy : proxySet) {
                for(TestSlot testSlot: proxy.getTestSlots() ) {
                    ...
                    if(testSession != null) {
                        // Most test runs should have their test run UUID in the capabilities.  This makes it easy to line up tests with their run requests
                        if(testSession.getRequestedCapabilities().containsKey(AutomationConstants.UUID)) {
                            ...
                            // No UUID means we still need to count this against the total count and just not match it up against the run requests
        ...
        if(untrackedTests != 0) {
            log.info(String.format("Adding %d tests to the total in progress count %d",untrackedTests,currentlyUsedNodes));
