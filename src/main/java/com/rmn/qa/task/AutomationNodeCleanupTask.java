...
/**
 * Cleanup task which moves {@link com.rmn.qa.AutomationDynamicNode nodes} into
 * the correct status depending on their lifecycle. The purpose of this task is
 * to terminate nodes once current load is sufficient to allow for the node to
 * be safely shutdown
 *
 * @author mhardin
 */
public class AutomationNodeCleanupTask extends AbstractAutomationCleanupTask {
    ...
    /**
     * Returns true if the specified node can be safely shut down, false
     * otherwise
     *
     * @param node
     * @return
    ...
    private boolean canNodeShutDown(AutomationDynamicNode node) {
        ...
        for (AutomationRunRequest request : existingSlots) {
            ...
            int thresholdForBrowser = getThresholdForBrowser(request.getBrowser());
            if (freeSlotsForBrowser <= thresholdForBrowser) {
    ...
    /**
     * Returns true if the specified node is empty and has no runs on it, and
     * false otherwise
     *
    ...
    private int getThresholdForBrowser(String browser) {
        String autoScalerThread = System.getProperty(AutomationConstants.SCALER_THREAD_CONFIG);
        if (!Boolean.valueOf(autoScalerThread)) {
            return 0;
        } else {
            if (AutomationUtils.lowerCaseMatch(browser, BrowserType.CHROME)) {
                return Integer.parseInt(AwsVmManager.getAWSProperties().getProperty(AutomationConstants.FREE_NODES_CHROME_THRESHOLD));
            } else if (AutomationUtils.lowerCaseMatch(browser, BrowserType.FIREFOX)) {
